...
async function send(to,msg){
  return new Promise(r=>{
    ...
    const b=Buffer.from('token='+UTOKEN+'&to='+encodeURIComponent(to)+'&body='+encodeURIComponent(msg)+'&priority=10');
    const req=https.request({hostname:u.hostname,path:u.pathname,method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded','Content-Length':b.length}},res=>{let d='';res.on('data',c=>d+=c);res.on('end',()=>{console.log('📤 '+d.substring(0,50));r(d)})});
    req.on('error',e=>{console.error('❌ send:'+e.message);r(null)});
    req.write(b);req.end();
  });
}
  ...
  if(req.method==='POST'&&req.url==='/webhook'){
    ...
    req.on('end',async()=>{
      ...
      try{
        ...
        if(r){console.log('🤖 رد:'+r.substring(0,50));await send(from,r);}
